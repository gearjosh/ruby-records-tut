...
describe '#Song' do
  ...
  describe('#==') do
    it("is the same song if it has the same attributes as another song") do
      ...
      song2 = Song.new({:name => "Naima", :album_id => @album_id, :id => nil})
  ...
  describe('.all') do
    it("returns a list of all songs") do
      song = Song.new({:name => "Giant Steps", :album_id => 2, :id => nil})
      song.save()
      song2 = Song.new({:name => "Naima", :album_id => 3, :id => nil})
      song2.save()
  ...
  describe('.clear') do
    it("clears all songs") do
      song = Song.new({:name => "Giant Steps", :album_id => 4, :id => nil})
      song.save()
      song2 = Song.new({:name => "Naima", :album_id => 4, :id => nil})
      song2.save()
      Song.clear()
      expect(Song.all).to(eq([]))
    end
  ...
  describe('#update') do
    it("updates an song by id") do
      song = Song.new({:name => "Naima", :album_id => 4, :id => nil})
      song.save()
      song.update("Mr. P.C.", 4)
